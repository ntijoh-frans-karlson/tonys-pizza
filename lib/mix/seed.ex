defmodule Mix.Tasks.Seed do
  ...
  defp create_tables() do
    ...
    Postgrex.query!(DB, "CREATE TABLE recepies (id SERIAL, name VARCHAR(255) NOT NULL, ingridients VARCHAR(255) NOT NULL)", [], pool: DBConnection.ConnectionPool)
  end
  ...
  defp seed_data() do
    ...
    # Recepies
    Postgrex.query!(DB, "INSERT INTO recepies(name, ingridients) VALUES($1, $2)", ["Margherita", "tomato_sauce mozzarella basil"])
    Postgrex.query!(DB, "INSERT INTO recepies(name, ingridients) VALUES($1, $2)", ["Capricciosa", "tomato_sauce mozzarella ham mushroom artichoke"])
    Postgrex.query!(DB, "INSERT INTO recepies(name, ingridients) VALUES($1, $2)", ["Marinara", "tomato_sauce"])
    Postgrex.query!(DB, "INSERT INTO recepies(name, ingridients) VALUES($1, $2)", ["Quattro formaggi", "tomato_sauce parmesan peccorino gorgonsola"])
    Postgrex.query!(DB, "INSERT INTO recepies(name, ingridients) VALUES($1, $2)", ["Prosciutto e funghi", "tomato_sauce mozzarella ham mushroom"])
    Postgrex.query!(DB, "INSERT INTO recepies(name, ingridients) VALUES($1, $2)", ["Ortolana", "tomato_sauce mozzarella bell_pepper aubergine zucchini"])
    Postgrex.query!(DB, "INSERT INTO recepies(name, ingridients) VALUES($1, $2)", ["Quattro stagioni", "tomato_sauce mozzarella ham mushroom artichoke olives"])
    Postgrex.query!(DB, "INSERT INTO recepies(name, ingridients) VALUES($1, $2)", ["Diavola", "tomato_sauce mozzarella salami bell_pepper chili"])
  end

  defp print_data() do
    IO.puts("Print data")
    Postgrex.query!(DB, "SELECT * FROM pizzas",[], pool: DBConnection.ConnectionPool) |> IO.inspect()
    Postgrex.query!(DB, "SELECT * FROM recepies",[], pool: DBConnection.ConnectionPool) |> IO.inspect()
